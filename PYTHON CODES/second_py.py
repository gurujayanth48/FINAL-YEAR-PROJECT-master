...
import second_py2 as sec



def foo(myscore,myreq='INTJ'):
    con = sql.connect("my.db")
    con.execute("DROP TABLE IF EXISTS second");
    con.execute('create table if not exists second("id" integer primary key autoincrement,"username" text not null, "email" text not null, "file" text not null, "twitter" text not null, "score" integer not null)')

    #con = sql.connect("my.db")
    cur = con.cursor()
    p=myscore
    cur.execute("select * from first where score >= ?",(p,))
    ans = cur.fetchall()
    print("SELECTED",ans)
    for x in ans:
        ...
        tw=x[3]
        sc=x[4]
        cur.execute("select username,email,file from users where email=?",(q,))
        ans2=cur.fetchall()
        for x in ans2:
            #print(x[0],x[1],x[2])
            cur.execute("INSERT INTO second (username,email,file,twitter,score) VALUES (?,?,?,?,?)", (x[0],x[1],x[2],tw,sc))
        #print(ans2)
    #print(ans)
    print("SECOND SORTED TABLE FOR NEXT PHASE")
    ...
    for r in ans3:
        ...
        file1.write(r[4]+"\n")
        file1.close()
        file_list.append(r[4])
    con.close()
    print(file_list)
    ans4 = sec.zoo(myreq,file_list)
    print("result===============",ans4)
    return ans4
    #--------------------------------------------------------------------------------------------------------------------------
